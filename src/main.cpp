...
#if defined(NDEBUG)
# error "Satoshi cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdc17653c3f54210f15c8ae2cd2e4abf22477a6e0aac9a678436c8af659fbf438");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Satoshi: starting difficulty is 1 / 2^12
...
{
    // Satoshi: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Satoshi: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Satoshi: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1506220215, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "24 September 2017 Fears of dam collapse add to Puerto Rico misery";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 27 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04aae5045dfe5ec1ab6de4b5f22b0aa1c4b8fb668fd5fe53b1055800cedcf7613d3da442d6f0588ee3a3f7187dd3c45d57f6291601f4c9dbb472a29d031755a665") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1506220215;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1232776;
        ...
        assert(block.hashMerkleRoot == uint256("0xe5da160082a28ee3776bbc1f56515e1ff0674dcf5e757e1354b2776fb215c175"));
...
unsigned char pchMessageStart[4] = { 0x9f, 0x7f, 0x37, 0x64 }; // Satoshi: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SatoshiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SatoshiMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SatoshiMiner(CWallet *pwallet)
{
    printf("SatoshiMiner started\n");
    ...
    try { loop {
        ...
        printf("Running SatoshiMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SatoshiMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SatoshiMiner, pwallet));
}
